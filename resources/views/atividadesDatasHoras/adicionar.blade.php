...
@section('content')
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <fieldset>
                ...
                {{Form::text('data', $atividade->evento->dataInicio->format('d/m/Y') , array('class' => 'form-control', 'id' => 'dataAtividade')) }}
            </fieldset>
            <fieldset>
                <label for="horarioInicio">Horário de Início</label>
                {{Form::text('horarioInicio', '12:00', array('class' => 'form-control', 'id' => 'horaInicioAtividade'))}}
            </fieldset>
            <fieldset>
                <label for="horarioTermino">Horário de Término</label>
                {{Form::text('horarioTermino', '13:00', array('class' => 'form-control', 'id' => 'horaTerminoAtividade'))}}
            </fieldset>
        </div>
    ...
    <script>
        ...
        $('#dataAtividade').datetimepicker({
            format: 'DD/MM/YYYY',
            locale: 'pt-br',
            minDate: '{{ $atividade->evento->dataInicio->format('m-d-Y H:i') }}',
            maxDate: '{{ $atividade->evento->dataTermino->format('m-d-Y H:i') }}',
            useCurrent: false
        });
    </script>
@endsection